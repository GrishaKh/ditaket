...
export const events = pgTable(
  "events",
  {
    ...
    // Optional device-reported GPS snapshot. Captured once at submit time only
    // if the reporter explicitly consents via the browser permission prompt.
    // No automated verification is performed against station coords (admin
    // moderator decides). All four fields are NULL when the reporter declined
    // or geolocation was unavailable.
    reporterLat: real("reporter_lat"),
    reporterLng: real("reporter_lng"),
    reporterAccuracyM: real("reporter_accuracy_m"),
    reporterGeoCapturedAt: timestamp("reporter_geo_captured_at"),
    reporterGeoConsentVersion: text("reporter_geo_consent_version"),
  ...
  (t) => ({
    ...
    moderationIdx: index("events_moderation_idx").on(
      t.moderationStatus,
      t.createdAt,
    ),
    fingerprintIdx: index("events_fingerprint_idx").on(
      t.reporterFingerprint,
      t.createdAt,
    ),
  }),
);
